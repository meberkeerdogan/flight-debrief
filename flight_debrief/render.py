"""Visualization of approach data and stability events."""
...
EVENT_SHADE_ALPHA = 0.18
EVENT_MARKER_ALPHA = 0.12
...
def make_plot_figure(
    ...
    """
    Create multi-panel approach visualization with event highlighting.

    Generates a figure with 5 subplots:
    - IAS with target speed band
    - Vertical speed with sink rate limit
    - Roll angle with bank limits
    - Pitch standard deviation with limit
    - Altitude AGL with gate marker

    Detected events are shaded on the relevant subplot.

    Args:
        approach: Preprocessed approach DataFrame
        events: List of detected violation events
        profile: Aircraft profile with thresholds
        target_speed: Estimated target approach speed
    ...
    # Map rules to their respective axes for event shading
    rule_to_ax = {
    ...
    # Shade events on relevant subplots
    for e in events:
        ax = rule_to_ax.get(e.rule)
        if ax is not None:
            ax.axvspan(e.t_start, e.t_end, alpha=EVENT_SHADE_ALPHA, color="grey")

        # Mark event boundaries on AGL subplot
        ax_agl.axvline(e.t_start, alpha=EVENT_MARKER_ALPHA, color="grey", linewidth=1.0)
        ax_agl.axvline(e.t_end, alpha=EVENT_MARKER_ALPHA, color="grey", linewidth=1.0)
